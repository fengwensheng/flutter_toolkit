...
#define LOGD(FORMAT, ...) __android_log_print(ANDROID_LOG_DEBUG,"Nightmare------>>",FORMAT,##__VA_ARGS__);
...
void videoStreamPlay(char *videoPath) {
    dart_print("调用成功");
...
int read_socket_buffer(void *opaque, uint8_t *buf, int buf_size) {
    SocketConnection *socketConnection = static_cast<SocketConnection *>(opaque);
    int count = socketConnection->recv_from_(buf, buf_size);
...
Java_ffmpeg_FFmpegNativeUtil_videoStreamPlay(JNIEnv *env, jobject instance,
