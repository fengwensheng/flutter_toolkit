    ...
    if (stream->has_pending || is_config) {
        ...
        if (stream->has_pending) {
            ...
            if (av_grow_packet(&stream->pending, packet->size)) {
                LOGD("Could not grow packet");
                return false;
            }
        } else {
            offset = 0;
            if (av_new_packet(&stream->pending, packet->size)) {
                LOGD("Could not create packet");
                return false;
            }
...
run_stream(void *data) {
    ...
    if (stream->decoder && !decoder_open(stream->decoder, codec)) {
        LOGD("Could not open decoder");
        goto finally_free_codec_ctx;
    }

    if (stream->recorder) {
        if (!recorder_open(stream->recorder, codec)) {
            LOGD("Could not open recorder");
            goto finally_close_decoder;
        }

        if (!recorder_start(stream->recorder)) {
            LOGD("Could not start recorder");
    ...
    if (!stream->parser) {
        LOGD("Could not initialize parser");
